...
PATCH_FILE="$ROOT_DIR/patches/openssl/slh-dsa-tls-signature-schemes.patch"
...
if [ ! -f "$OPENSSL_SOURCE_DIR/Configure" ]; then
    echo "OpenSSL checkout not found at $OPENSSL_SOURCE_DIR" >&2
...
if patch --force --silent --dry-run --reverse -d "$OPENSSL_SOURCE_DIR" -p1 \
    ...
    echo "SLH-DSA TLS is already enabled in $OPENSSL_SOURCE_DIR"
    exit 0
fi
...
echo "Enabled SLH-DSA TLS in $OPENSSL_SOURCE_DIR"
